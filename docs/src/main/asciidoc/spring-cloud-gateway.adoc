...
=== The `BasicAuth` `GatewayFilter` Factory

The `BasicAuth` `GatewayFilter` Factory relays Basic Authorization credentials to a route.
It will not authenticate requests.
It will not return an `HTTP 401 Unauthorized` status line with a `WWW-Authenticate` header for unauthenticated requests.

To use it, you must first base64 encode the username and password.

The following example configures a `BasicAuth` `GatewayFilter`:
  ...
  cloud:
    gateway:
      ...
      - id: basic_auth
        uri: https://example.org
        filters:
        - BasicAuth=dGVzdF91c2VyOnRlc3RfcGFzc3dvcmQ=
----
====

This adds `Authorization: Basic dGVzdF91c2VyOnRlc3RfcGFzc3dvcmQ=` header to the request's headers for all matching requests.

The following does the same thing in Java:
...
public RouteLocator routes(RouteLocatorBuilder builder) {
    ...
    return builder.routes()
        .route("basic_auth", r -> r.predicate(p -> true)
            .filters(f -> f
                 .basicAuth(encodedCredentials))
    .uri(uri))
    .build();
}
----
